...
lazy_static! {
    static ref COM_INIT: Result<i32, i32> = {
        initialize_com()
    };
}
...
impl fmt::Debug for SetupInstance {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "Setup Instance {{ instance_id: {:?}, installation_name: {:?}, installation_path: {:?}, installation_version: {:?}, product_path: {:?}}}",
            self.instance_id(),
...
fn displayRes(res: Result<OsString, i32>) -> String {
    match res {
        Ok(s) => match s.into_string() {
            Ok(s) => s,
            Err(_) => "Error".into(),
        },
        Err(i) => format!("Error: {}", i),
    }
}

impl fmt::Display for SetupInstance {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "Instance id: {}, Installation name: {}, Installation path: {}, Installation version: {}, Product path: {}", 
        displayRes(self.instance_id()),
        displayRes(self.installation_name()),
        displayRes(self.installation_path()),
        displayRes(self.installation_version()),
        displayRes(self.product_path())
        )
    }
}
